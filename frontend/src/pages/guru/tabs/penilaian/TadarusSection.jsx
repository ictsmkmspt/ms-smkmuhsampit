...
/**
 * Tadarus (membaca Al-Quran, bukan menghafal) — dicatat per surah +
 * rentang ayat. Murni catatan progres + keterangan, tanpa skor.
 *
 ...
 * riwayatnya + tambah catatan baru langsung untuknya.
 */
export default function TadarusSection() {
  ...
  const loadClassScores = (classId) => {
    api.get('/tadarus-scores', { params: { class_room_id: classId } }).then((res) => setClassScores(res.data));
  };
  ...
  const loadSiswaHistory = (studentId) => {
    ...
    api.get('/tadarus-scores', { params: { student_id: studentId } })
  ...
  const handleDelete = async (row) => {
    if (!confirm(`Hapus catatan Tadarus ${row.student?.user?.name || selectedSiswa?.user?.name} (${namaSurah(row.surah)} ${row.ayat_mulai}-${row.ayat_selesai})?`)) return;
    ...
    return (
      <div className="surface-card p-5">
        ...
        {loadingHistory ? (
          ...
          <p className="text-center text-ink-300 py-6 text-sm">Belum ada catatan Tadarus untuk siswa ini.</p>
        ) : (
