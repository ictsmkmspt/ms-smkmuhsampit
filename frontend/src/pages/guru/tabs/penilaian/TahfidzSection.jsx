...
import { Plus, Pencil, Trash2, X, BookMarked } from 'lucide-react';
 ...
 * benar-benar dihafal (bukan langsung pilih nomor juz — juz cuma
 * pembagian 1/30 volume mushaf, tidak selaras batas surah, sedangkan
 * hafalan sebenarnya dijalani per surah/ayat). Murni catatan progres +
 * keterangan, tanpa skor. Pola sama seperti Tahsin/Tadarus: tiap siswa
 * punya isian sendiri-sendiri, baris kosong dilewati saat simpan.
 */
export default function TahfidzSection() {
  ...
  const loadScores = () => {
    setLoading(true);
    api.get('/tahfidz-scores', { params: { class_room_id: classFilter || undefined } })
  ...
  const handleSimpanSemua = async (e) => {
    ...
    try {
      ...
      setError(msgs ? Object.values(msgs).flat().join(', ') : err.response?.data?.message || 'Gagal menyimpan catatan Tahfidz.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (row) => {
    if (!confirm(`Hapus catatan Tahfidz ${row.student?.user?.name} (${namaSurah(row.surah)} ${row.ayat_mulai}-${row.ayat_selesai})?`)) return;
  ...
  return (
    <div>
      <div className="surface-card p-4 mb-4 flex flex-col sm:flex-row sm:items-end gap-3">
        ...
        <button onClick={bukaForm} className="btn-primary text-sm shrink-0"><Plus className="w-4 h-4" /> Tambah Setoran Tahfidz</button>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && batchList.length === 0 && (
                <tr><td colSpan="3" className="py-8 text-center text-ink-300">{classFilter ? 'Tidak ada catatan untuk kelas ini.' : 'Belum ada catatan Tahfidz.'}</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* ===== Modal: tambah setoran untuk banyak siswa ===== */}
      {showForm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-line-200 shrink-0">
              <h3 className="font-display font-semibold text-ink-900 flex items-center gap-2"><BookMarked className="w-4 h-4 text-brand-600" /> Tambah Setoran Tahfidz</h3>
            ...
            <form onSubmit={handleSimpanSemua} className="p-5 overflow-y-auto space-y-3">
              ...
              <p className="text-xs text-ink-400">Isi surah & rentang ayat yang dihafal per siswa hari ini — siswa yang tidak diisi otomatis DILEWATI.</p>
