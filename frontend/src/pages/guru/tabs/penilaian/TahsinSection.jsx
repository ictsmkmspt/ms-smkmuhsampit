 ...
 * progres + keterangan (tanpa skor). Beda dari Nilai Akademik: tiap siswa
 * punya jilid/halaman SENDIRI-SENDIRI (bukan 1 kegiatan bersama), jadi
 * baris kosong dilewati saat simpan. Guru bebas pilih kelas manapun (tidak
...
export default function TahsinSection() {
  ...
  const loadScores = () => {
    setLoading(true);
    api.get('/tahsin-scores', { params: { class_room_id: classFilter || undefined } })
  ...
  const handleSimpanSemua = async (e) => {
    ...
    const entries = formStudents
      ...
      .filter((x) => x.halaman !== undefined && x.halaman !== '');

    if (entries.length === 0) { setError('Isi minimal 1 siswa (jilid dan halaman).'); return; }

    const belumLengkap = entries.some((x) => !x.jilid);
    if (belumLengkap) { setError('Lengkapi jilid untuk setiap siswa yang diisi halamannya.'); return; }

    setSaving(true);
    try {
      await api.post('/tahsin-scores/bulk', {
        tanggal: form.tanggal,
        entries: entries.map((x) => ({ ...x, jilid: Number(x.jilid), halaman: Number(x.halaman) })),
      });
      setShowForm(false);
      loadScores();
    } catch (err) {
      const msgs = err.response?.data?.errors;
      setError(msgs ? Object.values(msgs).flat().join(', ') : err.response?.data?.message || 'Gagal menyimpan catatan Tahsin.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (row) => {
    if (!confirm(`Hapus catatan Tahsin ${row.student?.user?.name} (Jilid ${row.jilid} Hal. ${row.halaman})?`)) return;
  ...
  return (
    <div>
      <div className="surface-card p-4 mb-4 flex flex-col sm:flex-row sm:items-end gap-3">
        ...
        <button onClick={bukaForm} className="btn-primary text-sm shrink-0"><Plus className="w-4 h-4" /> Tambah Catatan Tahsin</button>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && batchList.length === 0 && (
                <tr><td colSpan="3" className="py-8 text-center text-ink-300">{classFilter ? 'Tidak ada catatan untuk kelas ini.' : 'Belum ada catatan Tahsin.'}</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-line-200 shrink-0">
              <h3 className="font-display font-semibold text-ink-900 flex items-center gap-2"><BookOpen className="w-4 h-4 text-brand-600" /> Tambah Catatan Tahsin</h3>
            ...
            <form onSubmit={handleSimpanSemua} className="p-5 overflow-y-auto space-y-3">
              ...
              <p className="text-xs text-ink-400">Isi jilid & halaman per siswa yang maju hari ini — siswa yang tidak diisi otomatis DILEWATI.</p>
                ...
                {formStudents.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="flex items-center flex-wrap gap-2 px-3 py-2">
                      <span className="text-sm text-ink-900 min-w-0 flex-1 basis-32"><TruncateText text={s.user?.name} /></span>
                      <select value={isi.jilid ?? ''} onChange={(e) => setIsian(s.id, 'jilid', e.target.value)} className="field-input w-24 py-1.5 text-sm">
                        <option value="">Jilid</option>
                        {JILID_OPTIONS.map((j) => <option key={j} value={j}>Jilid {j}</option>)}
                      </select>
                      <input type="number" min="1" placeholder="Halaman" value={isi.halaman ?? ''} onChange={(e) => setIsian(s.id, 'halaman', e.target.value)} className="field-input w-24 py-1.5 text-sm text-center" />
                    </div>
                  );
                })}
              ...
              {viewingBatch?.items.map((row) => (
                <div key={row.id} className="flex items-center justify-between gap-3 px-5 py-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-ink-500">Jilid {row.jilid} &middot; Hal. {row.halaman}{row.keterangan ? ` · ${row.keterangan}` : ''}</p>
                  </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm">
            ...
            <form onSubmit={handleSaveEdit} className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <select value={editForm.jilid} onChange={(e) => setEditForm({ ...editForm, jilid: e.target.value })} className="field-input" required>
                  {JILID_OPTIONS.map((j) => <option key={j} value={j}>Jilid {j}</option>)}
                </select>
                <input type="number" min="1" placeholder="Halaman" value={editForm.halaman} onChange={(e) => setEditForm({ ...editForm, halaman: e.target.value })} className="field-input" required />
              </div>
