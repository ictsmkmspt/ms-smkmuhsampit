...
// Latihan tidak punya jadwal — diisi rentang sangat lebar di belakang
// layar (form-nya disembunyikan dari guru) supaya backend tetap bisa
// pakai validasi & logic isOpenForAttempt() yang sama tanpa kasus khusus.
...
export default function BuatUjianSubTab() {
  ...
  const [editingExam, setEditingExam] = useState(null); // null = Buat Ujian baru, terisi = Edit Ujian
  ...
  useEffect(() => {
    ...
    api.get('/cbt-materi').then((res) => setMateriOptions(res.data));
  }, []);
  ...
  // Dikelompokkan per Bank Soal — 1 mapel bisa punya banyak Bank Soal,
  // daftar soal flat akan membingungkan tanpa pengelompokan ini.
  ...
  // Langsung tampilkan semua ujian/latihan milik guru ini (lintas mapel &
  // kelas) begitu halaman dibuka — filter di bawah cuma penyaring.
  const loadExams = () => {
    setLoading(true);
  ...
  // Edit SENGAJA tetap boleh walau ujian ini sudah dikerjakan siswa —
  // dipakai kalau ada kesalahan (soal keliru, jadwal salah) yang perlu
  // diperbaiki. Ambil detail lengkap dulu (myExams()/index() cuma kirim
  // hitungan soal, bukan daftar question_id-nya) supaya form ke-prefill
  // dengan soal yang sudah dipilih sebelumnya. Mata pelajaran & tipe TIDAK
  // bisa diubah (mengikuti aturan update() backend), makanya tidak
  // di-reset di sini.
  const bukaEdit = async (exam) => {
    ...
    setShowForm(true);
    setBankQuestions([]);
    const res = await api.get(`/cbt-exams/${exam.id}`);
  ...
  useEffect(() => {
    ...
    api.get('/cbt-questions', { params: { subject_id: form.subject_id } }).then((res) => setBankQuestions(res.data));
  }, [showForm, form.subject_id]);
  ...
  const handleSimpan = async (e) => {
    ...
    try {
      ...
      if (editingExam) {
        await api.put(`/cbt-exams/${editingExam.id}`, payload);
      } else {
  ...
  const handlePublish = async (exam) => {
    try {
      await api.post(`/cbt-exams/${exam.id}/publish`);
      loadExams();
    } catch (err) {
  ...
  const handleClose = async (exam) => {
    ...
    try {
      await api.post(`/cbt-exams/${exam.id}/close`);
      loadExams();
    } catch (err) {
  ...
  // Sengaja tetap boleh dihapus walau sudah dikerjakan siswa — tapi
  // pesan konfirmasi dibuat lebih tegas untuk kasus itu karena semua
  // jawaban & skor siswa ikut terhapus PERMANEN (tidak bisa dibatalkan).
  const handleDelete = async (exam) => {
    ...
    try {
      await api.delete(`/cbt-exams/${exam.id}`);
      loadExams();
    } catch (err) {
  ...
  // Ujian (tipe ujian) cuma bisa "selesai" karena jadwal_selesai-nya
  // sudah lewat — buka kembali tanpa jadwal baru percuma (siswa tetap
  // ditolak & command penutup otomatis bakal langsung menutupnya lagi),
  // makanya guru wajib isi jadwal_selesai baru lewat modal ini. Latihan
  // tidak punya jadwal, jadi cukup konfirmasi biasa.
  const handleReopen = (exam) => {
    ...
    api.post(`/cbt-exams/${exam.id}/reopen`).then(loadExams).catch((err) => {
  ...
  const submitReopenUjian = async (e) => {
    ...
    try {
      await api.post(`/cbt-exams/${reopenExam.id}/reopen`, { jadwal_selesai: reopenJadwalSelesai });
      setReopenExam(null);
      loadExams();
    } catch (err) {
      const msgs = err.response?.data?.errors;
  ...
  const handleDuplicate = async (exam) => {
    ...
    try {
      await api.post(`/cbt-exams/${exam.id}/duplicate`);
      loadExams();
    } catch (err) {
  ...
  const handleRegenerateToken = async (exam) => {
    ...
    try {
      await api.post(`/cbt-exams/${exam.id}/regenerate-token`);
      loadExams();
    } catch (err) {
  ...
  const handlePublikasiNilai = async (exam) => {
    ...
    try {
      await api.post(`/cbt-exams/${exam.id}/publikasi-nilai`);
      loadExams();
    } catch (err) {
  ...
  const handleHentikanSemua = async (exam) => {
    ...
    try {
      const res = await api.post(`/cbt-exams/${exam.id}/hentikan-semua`);
  ...
  const bukaAttempts = async (exam) => {
    setViewingExam(exam);
    const res = await api.get(`/cbt-exams/${exam.id}/attempts`);
      ...
      <div className="space-y-2">
        ...
        {!loading && exams.length === 0 && (
          <p className="text-center text-sm text-ink-300 py-8">
            {subjectFilter || classFilter ? 'Tidak ada ujian yang cocok dengan saringan ini.' : 'Belum ada ujian dibuat.'}
          </p>
        )}
      </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col">
            ...
            <form onSubmit={handleSimpan} className="p-5 overflow-y-auto space-y-3">
              ...
              {form.tipe === 'latihan' && (
                <div>
                  ...
                  {form.subject_id && materiOptionsForSubject.length === 0 && (
                    <p className="text-xs text-ink-400 mt-1">Belum ada materi untuk mapel ini — buat dulu di menu Materi kalau mau dilampirkan.</p>
                  )}
              ...
              <div className="space-y-3">
                ...
                {form.tipe === 'ujian' && (
                  // Baris sendiri (bukan digabung 1 grid 4 kolom dengan
                  // Durasi/KKM) — datetime-local butuh lebar minimal
                  // supaya tanggal+jamnya tidak terpotong, baik di HP
                  // maupun desktop.
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
              ...
              <div>
                ...
                <div className="border border-line-200 rounded-xl overflow-hidden max-h-52 overflow-y-auto">
                  ...
                  {form.subject_id && bankQuestions.length === 0 && (
                    <p className="text-sm text-ink-300 text-center py-6">Bank soal mapel ini masih kosong — tambah dulu di tab Bank Soal.</p>
                  )}
                  {!form.subject_id && (
