...
function EssayModal({ exam, onClose, onNilaiTersimpan }) {
  ...
  const loadSiswa = () => {
    setLoading(true);
    api.get(`/admin-cbt-essay/${exam.id}/koreksi`).then((res) => {
  ...
  const bukaSiswa = async (attemptRow) => {
    ...
    const res = await api.get(`/admin-cbt-essay/${exam.id}/koreksi/${attemptRow.attempt_id}`);
  ...
  const simpanNilai = async (jawabanId) => {
    ...
    try {
      await api.put(`/admin-cbt-essay/jawaban/${jawabanId}/nilai`, { nilai });
...
// Laporan — versi Admin/Waka Kurikulum untuk guru yang dipilih di navbar
// Portal CBT (teacherId dari CbtAdminPortal). Mengikuti LaporanSubTab.jsx
// (versi guru) apa adanya, cuma endpoint diganti /admin-cbt-exams &
// /admin-cbt-essay (teacher_id eksplisit, tanpa batas kepemilikan).
export default function AdminLaporanTab({ teacherId }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const fetchAttempts = () => {
      api.get(`/admin-cbt-exams/${exam.id}/attempts`).then((res) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    if (!exam) return;
    api.get(`/admin-cbt-exams/${exam.id}/item-analysis`).then((res) => setAnalysis(res.data));
  }, [exam]);
  ...
  const handleReset = async (attempt) => {
    ...
    try {
      await api.post(`/admin-cbt-exams/attempts/${attempt.id}/reset`);
      refreshAttempts();
    } catch (err) {
  ...
  const handleTambahWaktu = async (attempt) => {
    ...
    try {
      await api.post(`/admin-cbt-exams/attempts/${attempt.id}/tambah-waktu`, { menit });
      refreshAttempts();
    } catch (err) {
  ...
  const handleExportExcel = () => {
    api.get(`/admin-cbt-exams/${exam.id}/export-nilai`, { responseType: 'blob' }).then((res) => {
  ...
  const handleKirimNilaiAkademik = async () => {
    ...
    try {
      const res = await api.post(`/admin-cbt-exams/${exam.id}/kirim-nilai-akademik`);
