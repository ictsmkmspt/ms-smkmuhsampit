...
// Jadwal — versi Admin/Waka Kurikulum untuk guru yang dipilih di navbar
// Portal CBT (teacherId dari CbtAdminPortal). Mengikuti JadwalSubTab.jsx
// (versi guru) apa adanya, cuma sumber datanya diganti /admin-cbt-exams
// (teacher_id eksplisit).
export default function AdminJadwalTab({ teacherId }) {
  const [exams, setExams] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    api.get('/admin-cbt-exams', { params: { teacher_id: teacherId } }).then((res) => setExams(res.data)).finally(() => setLoading(false));
  }, [teacherId]);
  ...
  return (
    <div className="space-y-5">
      ...
      {dikelompokkan.length === 0 && (
        <p className="text-center text-sm text-ink-300 py-8">Belum ada ujian terjadwal untuk guru ini.</p>
      )}
    </div>
  );
}
