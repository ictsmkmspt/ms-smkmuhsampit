...
// Kelola akun Pengawas Ujian — pola sama persis dengan PustakawanTab. Akun
// ini cuma dipakai login ke Portal CBT (/ujian/login) untuk memantau semua
// ujian/latihan yang sedang berlangsung lintas guru, tidak punya menu SIM
// sendiri.
export default function PengawasUjianTab() {
  ...
  const load = () => api.get('/pengawas-ujian-accounts').then((res) => setStaff(res.data));
  useEffect(() => { load(); }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await api.put(`/pengawas-ujian-accounts/${editId}`, { name: form.name, email: form.email });
      } else {
        await api.post('/pengawas-ujian-accounts', form);
      }
      setForm(FORM_KOSONG);
      setEditId(null);
  ...
  const handleResetPassword = async (id, name) => {
    ...
    try {
      const res = await api.put(`/pengawas-ujian-accounts/${id}/reset-password`);
  ...
  return (
    <div className="space-y-6">
      <div className="surface-card p-4 border-l-4 border-l-brand-400">
        <p className="text-sm text-ink-700">Akun ini login lewat Portal CBT (menu Ujian) untuk memantau SEMUA ujian & latihan yang sedang berlangsung dari guru manapun, dan bisa menghentikan sesi mencurigakan/macet atau menambah waktu kalau ada gangguan teknis.</p>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="surface-card p-5">
          <h2 className="font-display font-semibold text-ink-900 mb-1">{editId ? 'Ubah Akun' : 'Tambah Akun Pengawas Ujian'}</h2>
        ...
        <div className="table-scroll">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {staff.length === 0 && (
                <tr><td colSpan={3} className="py-6 text-center text-ink-300 whitespace-nowrap px-2">Belum ada akun Pengawas Ujian.</td></tr>
              )}
            </tbody>
          </table>
        </div>
