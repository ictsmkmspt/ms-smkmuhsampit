...
// TP & Materi — versi Admin/Waka Kurikulum, item navbar Portal CBT untuk
// GURU yang sudah dipilih (prop teacherId dari CbtAdminPortal). Mengikuti
// struktur MateriSubTab.jsx (versi guru) apa adanya, cuma endpoint & sumber
// daftar mapel diganti supaya dibatasi ke teacherId eksplisit.
export default function AdminMateriTab({ teacherId }) {
  ...
  const loadTp = () => {
    setLoading(true);
    api.get('/admin-cbt-tp', { params: { teacher_id: teacherId } }).then((res) => setTpList(res.data)).finally(() => setLoading(false));
  };
  const loadMateri = () => {
    api.get('/admin-cbt-materi', { params: { teacher_id: teacherId } }).then((res) => setMateriList(res.data));
  };

  useEffect(() => { loadTp(); loadMateri(); }, [teacherId]); // eslint-disable-line
  ...
  const simpanTp = async (e) => {
    ...
    try {
      if (editingTp) {
        await api.put(`/admin-cbt-tp/${editingTp.id}`, { judul: tpForm.judul });
      } else {
        await api.post('/admin-cbt-tp', { ...tpForm, teacher_id: teacherId });
      }
      setShowTpForm(false);
      loadTp();
    } catch (err) {
  ...
  const hapusTp = async (tp) => {
    ...
    try {
      await api.delete(`/admin-cbt-tp/${tp.id}`);
      loadTp();
    } catch (err) {
  ...
  const simpanMateri = async (e) => {
    ...
    try {
      ...
      if (editingMateri) {
        await api.put(`/admin-cbt-materi/${editingMateri.id}`, payload);
        if (gambarFile) {
          const fd = new FormData();
          fd.append('gambar', gambarFile);
          await api.post(`/admin-cbt-materi/${editingMateri.id}/gambar`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        }
      } else {
        const fd = new FormData();
        ...
        await api.post('/admin-cbt-materi', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
  ...
  const hapusMateri = async (m) => {
    ...
    try {
      await api.delete(`/admin-cbt-materi/${m.id}`);
      loadMateri();
    } catch (err) {
              ...
              <form onSubmit={simpanMateri} className="p-5 overflow-y-auto space-y-3">
                ...
                {editingMateri?.latihan_count > 0 && (
                  <p className="text-xs text-ink-500 bg-mist-50 border border-line-200 rounded-lg px-3 py-2">
                    Materi ini punya {editingMateri.latihan_count} latihan terhubung — atur/lepas keterhubungannya lewat menu Buat Ujian guru pemiliknya.
                  </p>
                )}
      ...
      {assignments.length === 0 && !loading ? (
        ...
        <div className="space-y-5">
          ...
          {!loading && tpList.length === 0 && (
            <p className="text-center text-sm text-ink-300 py-8">Belum ada TP dibuat untuk guru ini.</p>
          )}
        </div>
      )}
