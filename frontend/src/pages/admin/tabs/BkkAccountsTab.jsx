...
export default function BkkAccountsTab() {
  const { user } = useAuth();
  ...
  const loadAccounts = () => api.get('/bkk-account').then((res) => setAccounts(res.data));
  ...
  const handleAdd = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/bkk-account', form);
      setForm({ name: '', email: '' });
      ...
      setError(msgs ? Object.values(msgs).flat().join(', ') : 'Gagal menambah akun Pengurus BKK.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, name) => {
    if (!confirm(`Hapus akun Pengurus BKK "${name}"?`)) return;
    try {
      await api.delete(`/bkk-account/${id}`);
      loadAccounts();
    } catch (err) {
      alert(err.response?.data?.message || 'Gagal menghapus akun Pengurus BKK.');
    }
  };

  const handleResetPassword = async (id, name) => {
    if (!confirm(`Reset password akun Pengurus BKK "${name}" ke default (123456)?`)) return;
    try {
      const res = await api.put(`/bkk-account/${id}/reset-password`);
  ...
  const handleSaveEdit = async (id) => {
    setSaving(true);
    try {
      await api.put(`/bkk-account/${id}`, editData);
      setEditId(null);
      loadAccounts();
    } catch (err) {
      const msgs = err.response?.data?.errors;
    ...
    <div className="space-y-6">
      {!isAdmin && (
        <div className="surface-card p-4 border-l-4 border-l-brand-400">
          <p className="text-sm text-ink-700">Data Akun Pengurus BKK hanya bisa dilihat di sini — perubahan akun Pengurus BKK dikelola oleh Admin.</p>
        </div>
      )}

      {isAdmin && showForm && (
      <form onSubmit={handleAdd} className="surface-card p-5">
        <h2 className="font-display font-semibold text-ink-900 mb-1">Tambah Akun Pengurus BKK Baru</h2>
        <p className="text-xs text-ink-500 mb-4">
          Akun Pengurus BKK dipakai untuk mengelola menu Bursa Kerja Khusus (verifikasi loker, lamaran, tracer study, mitra kerja sama, cetak dokumen, laporan penempatan). Password akun otomatis dibuat "123456" — wajib diganti saat login pertama.
        </p>
        ...
        <div className="flex gap-2 mt-4">
          <button disabled={loading} className="btn-primary">
            <Plus className="w-4 h-4" /> {loading ? 'Menyimpan...' : 'Tambah Akun Pengurus BKK'}
          </button>
      ...
      <div className="surface-card p-5">
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          <h2 className="font-display font-semibold text-ink-900">
            Daftar Akun Pengurus BKK <span className="text-ink-500 font-sans font-normal text-sm">({accounts.length})</span>
          </h2>
          {isAdmin && !showForm && (
            <button onClick={() => { setForm({ name: '', email: '' }); emailManual.current = false; setError(''); setShowForm(true); }} className="btn-primary shrink-0">
              <Plus className="w-4 h-4" /> Tambah Akun Pengurus BKK
            </button>
          )}
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {accounts.length === 0 && (
              <tr><td colSpan="3" className="py-6 text-center text-ink-300 whitespace-nowrap px-2">Belum ada akun Pengurus BKK.</td></tr>
            )}
          </tbody>
        </table>
        </div>
