...
export default function BkAccountsTab() {
  const { user } = useAuth();
  ...
  const loadAccounts = () => api.get('/bk').then((res) => setAccounts(res.data));
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setError(msgs ? Object.values(msgs).flat().join(', ') : 'Gagal menambah akun BK.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async (id, name) => {
    if (!confirm(`Reset password akun BK "${name}" ke default (123456)?`)) return;
    try {
      const res = await api.put(`/bk/${id}/reset-password`);
    ...
    <div className="space-y-6">
      {!isAdmin && (
        <div className="surface-card p-4 border-l-4 border-l-brand-400">
          <p className="text-sm text-ink-700">Data Akun BK hanya bisa dilihat di sini — perubahan akun BK dikelola oleh Admin.</p>
        </div>
      )}
      ...
      <form onSubmit={handleAdd} className="surface-card p-5">
        ...
        <p className="text-xs text-ink-500 mb-4">
          Akun BK (Bimbingan Konseling) dipakai untuk menindaklanjuti siswa yang masuk tahap Sanksi Bertingkat, dan mencatat kasus bimbingan konseling. Password akun otomatis dibuat "123456" — wajib diganti saat login pertama.
        </p>
      ...
      <div className="surface-card p-5">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {accounts.length === 0 && (
              <tr><td colSpan="3" className="py-6 text-center text-ink-300 whitespace-nowrap px-2">Belum ada akun BK.</td></tr>
            )}
          </tbody>
        </table>
        </div>
