...
/**
 * Rekap Tahfidz — menu Laporan khusus Waka Kesiswaan, baca-saja. Sama pola
 * dengan TahsinReportTab: per siswa, kolom "Penilaian Terakhir", klik
 * "Detail" untuk lihat seluruh riwayat hafalannya.
 */
export default function TahfidzReportTab() {
  ...
  useEffect(() => {
    ...
    api.get('/tahfidz-scores', { params: { class_room_id: kelasFilter } }).then((res) => { if (!cancelled) setClassScores(res.data); });
  ...
  const selectSiswa = (s) => {
    ...
    api.get('/tahfidz-scores', { params: { student_id: s.id } })
  ...
  return (
    <div className="surface-card p-5">
      ...
      {!kelasFilter ? (
        <p className="text-sm text-ink-300 text-center py-8">Pilih kelas untuk melihat rekap Tahfidz siswa.</p>
      ) : loadingRoster ? (
