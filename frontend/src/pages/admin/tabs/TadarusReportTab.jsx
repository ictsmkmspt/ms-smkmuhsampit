 ...
 * dengan TahfidzReportTab.
 */
export default function TadarusReportTab() {
  ...
  useEffect(() => {
    ...
    api.get('/tadarus-scores', { params: { class_room_id: kelasFilter } }).then((res) => { if (!cancelled) setClassScores(res.data); });
  ...
  const selectSiswa = (s) => {
    ...
    api.get('/tadarus-scores', { params: { student_id: s.id } })
  ...
  return (
    <div className="surface-card p-5">
      ...
      {!kelasFilter ? (
        <p className="text-sm text-ink-300 text-center py-8">Pilih kelas untuk melihat rekap Tadarus siswa.</p>
      ) : loadingRoster ? (
