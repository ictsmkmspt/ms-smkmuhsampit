...
/**
 * Rekap kerja BK untuk Admin & Waka Kesiswaan — pemantauan, bukan tempat
 * BK bekerja sehari-hari (itu di dashboard /bk sendiri). Menggabungkan 2
 * sumber: kejadian Sanksi Bertingkat (otomatis) dan Catatan BK (manual).
 */
export default function RekapBkTab() {
  ...
  const hapusKejadian = async (k) => {
    ...
    try {
      await api.delete(`/sanksi-kejadian/${k.id}`);
      loadAll();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="surface-card p-5 flex flex-wrap gap-3 items-end">
        ...
        <p className="text-xs text-ink-400 ml-auto">
          {tahunAktif ? 'Tahun ajaran aktif.' : `Tahun ajaran ${tahunAjaranTerpilih?.nama || ''}.`}
        </p>
      </div>
