...
export default function PoinPelanggaranTab() {
  ...
  const handleResetAll = async () => {
    if (!confirm('Ini akan menghapus SEMUA riwayat pelanggaran dan mengembalikan poin SEMUA siswa ke 0. Tindakan ini PERMANEN dan tidak bisa dibatalkan. Lanjutkan?')) return;
    setResetLoading(true);
    setResetMessage('');
    try {
      const res = await api.post('/violations/reset-all', { confirm: true });
      setResetMessage(res.data.message);
      setResetConfirmText('');
    } catch (err) {
      alert(err.response?.data?.message || 'Gagal mereset poin pelanggaran.');
    } finally {
    ...
    <div className="space-y-6 max-w-xl">
      ...
      <div className="surface-card p-5 border-2 border-rose-200 bg-rose-50/40">
        ...
        <p className="text-sm text-ink-600 mb-4">
          Kembalikan poin pelanggaran <b>SEMUA siswa</b> ke 0 dan hapus <b>SELURUH riwayat pelanggaran</b> secara permanen (termasuk yang otomatis dari alpa). Biasanya dipakai di awal tahun pelajaran baru. <b>Tindakan ini tidak bisa dibatalkan.</b>
        </p>
        <div className="flex flex-wrap gap-3 items-center">
          ...
          >
            ...
            {resetLoading ? 'Mereset...' : 'Reset Semua Poin Pelanggaran'}
          </button>
        </div>
        {resetMessage && (
