  ...
  return (
    <div className="space-y-6">
      ...
      <div className="surface-card p-5">
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              disabled={isTeknisi ? rooms.length === 0 : assets.length === 0}
              className="flex items-center gap-1.5 text-sm font-medium text-ink-700 bg-mist-50 hover:bg-mist-100 disabled:opacity-50 disabled:cursor-not-allowed border border-line-200 rounded-xl px-4 py-2 transition shrink-0"
            >
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {assetTersaring.length === 0 && <tr><td colSpan={isTeknisi ? 14 : 13} className="py-6 text-center text-ink-300 whitespace-nowrap px-2">{assets.length === 0 ? 'Belum ada data aset.' : 'Tidak ada aset yang cocok dengan pencarian.'}</td></tr>}
          </tbody>
        </table>
        </div>
      ...
      {showExportKir && (
        <ExportKirModal
          rooms={rooms}
          fixedRoomId={isTeknisi ? undefined : user.room_id}
          onClose={() => setShowExportKir(false)}
        />
      )}
    </div>
  );
