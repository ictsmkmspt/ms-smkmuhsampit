...
import MateriTab from '../siswa/MateriTab';

const NAV = [
  { key: 'ujian', label: 'Ujian', icon: ClipboardEdit, component: UjianTab },
  { key: 'latihan', label: 'Latihan', icon: GraduationCap, component: LatihanTab },
  { key: 'materi', label: 'Materi', icon: BookOpen, component: MateriTab },
  { key: 'nilai', label: 'Nilai', icon: ClipboardList, component: NilaiSiswaTab },
];

export default function CbtSiswaPortal() {
  const { user } = useAuth();
  const [active, setActive] = useState('ujian');
    ...
    <div className="cbt-scope min-h-screen bg-mist-50 flex flex-col md:flex-row pb-16 md:pb-0">
      ...
      <div className="md:hidden bg-[#0B1B3A] px-4 py-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[10px] text-white/50 uppercase tracking-wide">Portal CBT &middot; Siswa</p>
      ...
      <main className="flex-1 min-w-0 px-4 py-5 md:px-8 md:py-6 w-full">
        <h2 className="cbt-display text-lg font-bold text-ink-900 mb-4 text-center md:text-left max-w-md mx-auto md:mx-0">{NAV.find((n) => n.key === active)?.label}</h2>
        {ActiveComponent && <ActiveComponent />}
      </main>

      {/* Tab bar rata — cuma tampil di HP */}
