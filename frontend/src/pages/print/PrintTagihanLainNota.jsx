...
export default function PrintTagihanLainNota() {
  ...
  useEffect(() => {
    if (!tagihanId) {
      setError('Parameter tidak lengkap (tagihan_id).');
      return;
    }
    api.get(`/tagihan-lain/${tagihanId}`)
  ...
  if (!tagihan) return <div className="p-8 text-center text-ink-400">Memuat data...</div>;
  ...
  return (
    <div className="p-4 bg-white text-ink-900 text-sm" style={{ maxWidth: '105mm' }}>
      ...
      <div className="mb-3 text-center">
        ...
        <h1 className="font-bold text-sm mt-1">NOTA PEMBAYARAN</h1>
      </div>
      ...
      <table className="w-full text-xs mb-3">
        <tbody>
          <tr>
            ...
            <td className="py-0.5">{tagihan.student?.user?.name}</td>
          </tr>
          <tr>
            <td className="pr-2 py-0.5 align-top">Kelas</td>
            <td className="pr-2 py-0.5 align-top">:</td>
            <td className="py-0.5">{tagihan.student?.class_room?.name || '-'}</td>
          </tr>
          <tr>
            <td className="pr-2 py-0.5 align-top">NIS</td>
            <td className="pr-2 py-0.5 align-top">:</td>
            <td className="py-0.5">{tagihan.student?.nis}</td>
          </tr>
          <tr>
            <td className="pr-2 py-0.5 align-top">Untuk</td>
            <td className="pr-2 py-0.5 align-top">:</td>
            <td className="py-0.5">{tagihan.nama_tagihan}</td>
          </tr>
          {tagihan.keterangan && (
            <tr>
              <td className="pr-2 py-0.5 align-top">Keterangan</td>
              <td className="pr-2 py-0.5 align-top">:</td>
              <td className="py-0.5">{tagihan.keterangan}</td>
            </tr>
          )}
          <tr>
            <td className="pr-2 py-0.5 align-top">Tanggal Bayar</td>
            <td className="pr-2 py-0.5 align-top">:</td>
            <td className="py-0.5">{formatTanggal(tagihan.tanggal_bayar)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="border-t border-b border-ink-400 py-2 mb-4 flex items-center justify-between text-xs">
        ...
        <span className="font-bold text-sm">{formatRupiah(tagihan.nominal)}</span>
      </div>
      ...
      <div className="flex justify-end text-xs">
        <div className="text-center">
          <p>Petugas TU,</p>
          <div className="h-10" />
          <p className="border-t border-ink-400 pt-1 inline-block px-4">{tagihan.dicatat_oleh?.name || '( ..................... )'}</p>
        </div>
      </div>
