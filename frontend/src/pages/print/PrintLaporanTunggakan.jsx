...
// Cetak Laporan Tunggakan dari menu Ringkasan TU — kalau class_room_id
// dikirim (lagi difilter 1 kelas di layar), cetak cuma kelas itu. Kalau
// tidak (lagi "Semua Kelas"), cetak SEMUA kelas sekaligus tapi dikelompokkan
// & diurutkan per kelas (bukan 1 tabel campur aduk) supaya gampang dibagi
// per wali kelas.
export default function PrintLaporanTunggakan() {
  const [params] = useSearchParams();
  const classRoomId = params.get('class_room_id') || '';
  const search = params.get('search') || '';
  const namaKelasFilter = params.get('nama_kelas') || '';
  ...
  useEffect(() => {
    const query = {};
    if (classRoomId) query.class_room_id = classRoomId;
    if (search) query.search = search;
    api.get('/laporan/tunggakan', { params: query })
  ...
  const kelasKelompok = useMemo(() => {
    ...
    for (const row of list) {
      const nama = row.student.class_room?.name || 'Tanpa Kelas';
      if (!map.has(nama)) map.set(nama, []);
      map.get(nama).push(row);
    }
    return [...map.entries()]
      .sort((a, b) => a[0].localeCompare(b[0], 'id'))
      .map(([nama, rows]) => ({
        nama,
        rows: rows.sort((a, b) => (a.student.user?.name || '').localeCompare(b.student.user?.name || '', 'id')),
        total: rows.reduce((sum, r) => sum + Number(r.total_tunggakan || 0), 0),
      }));
  }, [list]);

  const grandTotal = kelasKelompok.reduce((sum, k) => sum + k.total, 0);
  const totalSiswa = list?.length || 0;

  if (error) return <div className="p-8 text-center text-rose-600">{error}</div>;
  if (list === null) return <div className="p-8 text-center text-ink-400">Memuat data...</div>;

  const judul = classRoomId && namaKelasFilter
    ? `LAPORAN TUNGGAKAN — KELAS ${namaKelasFilter.toUpperCase()}`
    : 'LAPORAN TUNGGAKAN SELURUH KELAS';
  ...
  return (
    <div className="p-6 bg-mist-50 min-h-screen">
      ...
      <div className="lembar max-w-3xl mx-auto bg-white">
        ...
        <div className="lembar-body">
          <p className="text-xs text-ink-500 mb-4">
            {totalSiswa} siswa · Total Tunggakan {formatRupiah(grandTotal)}
          </p>

          {kelasKelompok.length === 0 ? (
            <p className="text-center text-brand-700 py-10 text-sm">Tidak ada siswa dengan tunggakan.</p>
          ) : (
            <>
              {kelasKelompok.map((kelas) => (
                <div key={kelas.nama} className="kelas-section">
                  <p className="kelas-judul">Kelas {kelas.nama}</p>
                    ...
                    <tbody>
                      {kelas.rows.map((row) => (
                        <tr key={row.student.id}>
                          ...
                          <td className="text-right font-semibold">{formatRupiah(row.total_tunggakan)}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr>
                        <td colSpan={4} className="text-right font-semibold">Subtotal {kelas.nama}</td>
              ...
              <div className="total-box">
                <span>Total Tunggakan Keseluruhan</span>
      ...
      <style>{`
        ...
        .tunggakan-tabel th:nth-child(3), .tunggakan-tabel td:nth-child(3),
